...
import {
  ...
  sha384,
  sha512
} from "./chunk-3LQ52NAQ.js";
import {
  keccak_256
} from "./chunk-3T47BPSV.js";
import {
  Hash,
  abytes,
  add,
...
var UserOperationReceiptNotFoundError = class extends BaseError {
  constructor({ hash: hash8 }) {
    super(`User Operation receipt with hash "${hash8}" could not be found. The User Operation may not have been processed yet.`, { name: "UserOperationReceiptNotFoundError" });
  }
};
var UserOperationNotFoundError = class extends BaseError {
  constructor({ hash: hash8 }) {
    super(`User Operation with hash "${hash8}" could not be found.`, {
...
var WaitForUserOperationReceiptTimeoutError = class extends BaseError {
  constructor({ hash: hash8 }) {
    super(`Timed out while waiting for User Operation with hash "${hash8}" to be confirmed.`, { name: "WaitForUserOperationReceiptTimeoutError" });
  }
};
...
async function getUserOperation(client, { hash: hash8 }) {
  const result = await client.request({
    method: "eth_getUserOperationByHash",
    params: [hash8]
  }, { dedupe: true });
  if (!result)
    throw new UserOperationNotFoundError({ hash: hash8 });
...
async function getUserOperationReceipt(client, { hash: hash8 }) {
  const receipt = await client.request({
    method: "eth_getUserOperationReceipt",
    params: [hash8]
  }, { dedupe: true });
  if (!receipt)
    throw new UserOperationReceiptNotFoundError({ hash: hash8 });
...
function waitForUserOperationReceipt(client, parameters) {
  const { hash: hash8, pollingInterval = client.pollingInterval, retryCount, timeout = 12e4 } = parameters;
  let count = 0;
    ...
    const unobserve = observe(observerId, { resolve, reject }, (emit) => {
      ...
      const unpoll = poll(async () => {
        if (retryCount && count >= retryCount)
          done(() => emit.reject(new WaitForUserOperationReceiptTimeoutError({ hash: hash8 })));
        try {
          const receipt = await getAction(client, getUserOperationReceipt, "getUserOperationReceipt")({ hash: hash8 });
      ...
      if (timeout)
        setTimeout(() => done(() => emit.reject(new WaitForUserOperationReceiptTimeoutError({ hash: hash8 }))), timeout);
      return unpoll;
    });
  });
}
...
// node_modules/viem/node_modules/ox/node_modules/@noble/curves/esm/abstract/modular.js
var _0n = BigInt(0);
var _1n = BigInt(1);
var _2n = BigInt(2);
var _3n = BigInt(3);
...
// node_modules/viem/node_modules/ox/node_modules/@noble/curves/esm/abstract/hash-to-curve.js
...
// node_modules/viem/node_modules/ox/node_modules/@noble/curves/esm/abstract/curve.js
var _0n2 = BigInt(0);
var _1n2 = BigInt(1);
...
// node_modules/viem/node_modules/ox/node_modules/@noble/curves/esm/abstract/weierstrass.js
...
function _splitEndoScalar(k, basis, n) {
  ...
  if (k2neg)
    k2 = -k2;
  const MAX_NUM = bitMask2(Math.ceil(bitLen2(n) / 2)) + _1n3;
...
function ecdh(Point, ecdhOpts = {}) {
  ...
  const lengths = Object.assign(getWLengths(Point.Fp, Fn), { seed: getMinHashLength2(Fn.ORDER) });
  ...
  return Object.freeze({ getPublicKey: getPublicKey4, getSharedSecret, keygen, Point, utils: utils2, lengths });
}
function ecdsa(Point, hash8, ecdsaOpts = {}) {
  ...
  const { keygen, getPublicKey: getPublicKey4, getSharedSecret, utils: utils2, lengths } = ecdh(Point, ecdsaOpts);
  ...
  function verify7(signature, message, publicKey, opts = {}) {
    ...
    const sig = format9 === void 0 ? tryParsingSig(signature) : Signature.fromBytes(ensureBytes2("sig", signature), format9);
    if (sig === false)
      return false;
    try {
...
// node_modules/viem/node_modules/ox/node_modules/@noble/curves/esm/_shortw_utils.js
function createCurve2(curveDef, defHash) {
  const create4 = (hash8) => weierstrass({ ...curveDef, hash: hash8 });
  return { ...create4(defHash), create: create4 };
}

// node_modules/viem/node_modules/ox/node_modules/@noble/curves/esm/nist.js
var p256_CURVE = {
...
// node_modules/viem/node_modules/ox/node_modules/@noble/curves/esm/p256.js
...
// node_modules/viem/node_modules/ox/node_modules/@noble/curves/esm/secp256k1.js
var secp256k1_CURVE = {
...
var secp256k12 = createCurve2({ ...secp256k1_CURVE, Fp: Fpk1, lowS: true, endo: secp256k1_ENDO }, sha256);
...
function schnorrGetExtPubKey(priv) {
  ...
  const d_ = _normFnElement2(Fn, priv);
...
function fromString4(value, options = {}) {
  return fromBytes2(encoder3.encode(value), options);
}
...
function toString3(hex, options = {}) {
  const { size: size6 } = options;
...
function fromDerHex(signature) {
  const { r, s } = secp256k1.Signature.fromDER(from5(signature).slice(2));
  return { r, s };
}
...
function toBytes5(signature) {
  return fromHex4(toHex6(signature));
}
function toHex6(signature) {
  assert4(signature);
  const r = signature.r;
  const s = signature.s;
  const signature_ = concat5(
...
function toTuple(signature) {
  ...
  return [
    yParity ? "0x01" : "0x",
    r === 0n ? "0x" : trimLeft3(fromNumber2(r)),
    s === 0n ? "0x" : trimLeft3(fromNumber2(s))
  ];
}
...
function fromString5(value, options = {}) {
  return fromBytes5(fromString3(value), options);
}
...
function toHex7(value) {
  return fromBytes2(toBytes6(value));
}
...
function ripemd1602(value, options = {}) {
  const { as = typeof value === "string" ? "Hex" : "Bytes" } = options;
  const bytes = ripemd160(from4(value));
  if (as === "Bytes")
    return bytes;
  return fromBytes2(bytes);
}
...
var p384_hasher2 = (() => {
  return createHasher(p3842.Point, createSWU2(p3842.Point, {
    A: p384_CURVE2.a,
    B: p384_CURVE2.b,
...
var p521_hasher2 = (() => {
  return createHasher(p5212.Point, createSWU2(p5212.Point, {
    A: p521_CURVE2.a,
    B: p521_CURVE2.b,
...
function getPublicKey(options) {
  ...
  const point = secp256r12.ProjectivePoint.fromPrivateKey(typeof privateKey === "string" ? privateKey.slice(2) : fromBytes2(privateKey).slice(2));
  return from6(point);
}
...
function sign3(options) {
  const { extraEntropy: extraEntropy3 = extraEntropy2, hash: hash8, payload, privateKey } = options;
  const { r, s, recovery } = secp256r12.sign(payload instanceof Uint8Array ? payload : fromHex4(payload), privateKey instanceof Uint8Array ? privateKey : fromHex4(privateKey), {
    extraEntropy: typeof extraEntropy3 === "boolean" ? extraEntropy3 : from5(extraEntropy3).slice(2),
    lowS: true,
    ...hash8 ? { prehash: true } : {}
  });
  return {
...
function verify2(options) {
  ...
  return secp256r12.verify(signature, payload instanceof Uint8Array ? payload : fromHex4(payload), toHex5(publicKey).substring(2), ...hash8 ? [{ prehash: true, lowS: true }] : []);
}
...
function getAuthenticatorData(options = {}) {
  ...
  return concat5(rpIdHash, flag_bytes, signCount_bytes);
}
...
function getSignPayload2(options) {
  ...
  const payload = concat5(authenticatorData, clientDataJSONHash);
...
function verify3(options) {
  ...
  if (fromBytes2(toBytes6(challenge_extracted)) !== challenge2)
    return false;
...
__export(Address_exports2, {
  InvalidAddressError: () => InvalidAddressError3,
...
function checksum2(address) {
  ...
  const hash8 = keccak2563(fromString3(hexAddress), { as: "Bytes" });
...
function encode2(preparedParameters) {
  ...
  return concat5(...staticParameters, ...dynamicParameters);
}
...
function encodeArray(value, options) {
  ...
  if (dynamic || dynamicChild) {
    ...
    if (dynamic) {
      ...
      return {
        dynamic: true,
        encoded: preparedParameters.length > 0 ? concat5(length2, data) : length2
      };
    }
    if (dynamicChild)
  ...
  return {
    dynamic: false,
    encoded: concat5(...preparedParameters.map(({ encoded }) => encoded))
  };
}
function encodeBytes(value, { type: type6 }) {
  ...
  if (!parametersize) {
    ...
    return {
      dynamic: true,
      encoded: concat5(padLeft3(fromNumber2(bytesSize, { size: 32 })), value_)
    };
  }
...
function encodeString(value) {
  ...
  return {
    dynamic: true,
    encoded: concat5(padRight(fromNumber2(size4(hexValue), { size: 32 })), ...parts)
  };
}
function encodeTuple(value, options) {
  ...
  return {
    dynamic,
    encoded: dynamic ? encode2(preparedParameters) : concat5(...preparedParameters.map(({ encoded }) => encoded))
  };
}
...
function encode4(abiConstructor, options) {
  ...
  return concat5(bytecode, ((_a = abiConstructor.inputs) == null ? void 0 : _a.length) && (args == null ? void 0 : args.length) ? encode3(abiConstructor.inputs, args) : "0x");
}
...
function encode5(abiError, ...args) {
  ...
  return data ? concat5(selector, data) : selector;
}
...
function fromTupleList(accessList) {
  ...
  for (let i = 0; i < accessList.length; i++) {
    ...
    list.push({
      address,
      storageKeys: storageKeys.map((key) => validate6(key) ? key : trimLeft3(key))
    });
  }
  return list;
}
...
function fromBytes6(bytes, options = {}) {
  const { as = "Bytes" } = options;
  return from16(bytes, { as });
}
function fromHex8(hex, options = {}) {
  const { as = "Hex" } = options;
...
function hash(authorization) {
  return keccak2563(concat5("0x05", fromHex8(toTuple2(authorization))));
}
...
// node_modules/ox/_esm/core/Base58.js
function fromBytes7(value) {
  return from18(value);
}
function fromHex9(value) {
  return from18(value);
}
function fromString6(value) {
  return from18(fromString3(value));
}
function toBytes8(value) {
  return fromHex4(toHex9(value));
}
...
function merkelize(tree) {
  function inner(node) {
    ...
    if (node.type === "internal") {
      ...
      return hash2(concat4(hash_left, hash_right));
    }
    let level = node.values.map(hash2);
    while (level.length > 1) {
      const level_ = [];
      for (let i = 0; i < level.length; i += 2)
        level_.push(hash2(concat4(level[i], level[i + 1])));
      level = level_;
    }
    return hash2(concat4(node.stem, new Uint8Array(1).fill(0), level[0]));
  }
...
var InvalidVersionedHashSizeError = class extends BaseError3 {
  constructor({ hash: hash8, size: size6 }) {
...
function contains(bloom, input) {
  ...
  const hash8 = keccak2563(input, { as: "Bytes" });
...
var _Field2 = class {
  ...
  isValid({ c0, c1 }) {
    function isValidC(num2, ORDER) {
      return typeof num2 === "bigint" && _0n6 <= num2 && num2 < ORDER;
    }
...
function getPublicKey2(options) {
  ...
  const { px, py, pz } = group.ProjectivePoint.fromPrivateKey(from5(privateKey).slice(2));
...
function sign6(options) {
  ...
  const payloadPoint = payloadGroup.hashToCurve(from4(payload), suite ? { DST: fromString3(suite) } : void 0);
...
function verify5(options) {
  ...
  const payloadPoint = group.hashToCurve(from4(payload), suite ? { DST: fromString3(suite) } : void 0);
...
function fromCreate(options) {
  const from32 = fromHex4(from9(options.from));
  let nonce = fromNumber3(options.nonce);
  if (nonce[0] === 0)
    nonce = new Uint8Array([]);
  return from9(`0x${keccak2563(fromBytes6([from32, nonce], { as: "Hex" })).slice(26)}`);
}
function fromCreate2(options) {
  const from32 = fromHex4(from9(options.from));
  const salt = padLeft2(validate2(options.salt) ? options.salt : fromHex4(options.salt), 32);
  const bytecodeHash = (() => {
  ...
  return from9(slice3(keccak2563(concat4(fromHex4("0xff"), from32, salt, bytecodeHash), { as: "Hex" }), 12));
}
...
function unwrapLabelhash(label) {
  ...
  if (!validate3(hash8, { strict: true }))
    return null;
  return hash8;
}
...
function namehash(name) {
  ...
  for (let i = labels.length - 1; i >= 0; i -= 1) {
    ...
    const hashed = hashFromEncodedLabel ? fromHex4(hashFromEncodedLabel) : keccak2563(fromString3(labels[i]), { as: "Bytes" });
...
function sign7(options) {
  const { extraEntropy: extraEntropy3 = extraEntropy2, hash: hash8, payload, privateKey } = options;
  const { r, s, recovery } = secp256k1.sign(from4(payload), from4(privateKey), {
    extraEntropy: typeof extraEntropy3 === "boolean" ? extraEntropy3 : from5(extraEntropy3).slice(2),
    lowS: true,
    ...hash8 ? { prehash: true } : {}
  });
...
function verify6(options) {
  ...
  return secp256k1.verify(signature, from4(payload), toBytes4(publicKey), ...hash8 ? [{ prehash: true, lowS: true }] : []);
}
...
function from22(response, options = {}) {
...
function from23(provider, options = {}) {
  var _a, _b;
...
function from26(value, decimals = 0) {
...
function fromEther(ether, unit = "wei") {
  return from26(ether, exponents.ether - exponents[unit]);
}
function fromGwei(gwei, unit = "wei") {
  return from26(gwei, exponents.gwei - exponents[unit]);
}
...
function serialize(envelope, options = {}) {
  ...
  if (signature) {
    ...
    serialized = [
      ...serialized,
      fromNumber2(v),
      signature.r === 0n ? "0x" : trimLeft3(fromNumber2(signature.r)),
      signature.s === 0n ? "0x" : trimLeft3(fromNumber2(signature.s))
    ];
  } else if (chainId > 0)
    serialized = [...serialized, fromNumber2(chainId), "0x", "0x"];
  return fromHex8(serialized);
}
...
function deserialize2(serialized) {
  const transactionArray = toHex8(slice3(serialized, 1));
...
function serialize2(envelope, options = {}) {
  ...
  return concat5(serializedType, fromHex8(serialized));
}
...
function serialize3(envelope, options = {}) {
  ...
  return concat5("0x01", fromHex8(serialized));
}
...
function assert9(envelope) {
  ...
  if (blobVersionedHashes) {
    ...
    for (const hash8 of blobVersionedHashes) {
      const size6 = size4(hash8);
      const version2 = toNumber(slice3(hash8, 0, 1));
      if (size6 !== 32)
        throw new InvalidVersionedHashSizeError({ hash: hash8, size: size6 });
...
function deserialize4(serialized) {
  const transactionOrWrapperArray = toHex8(slice3(serialized, 1));
...
function serialize4(envelope, options = {}) {
  ...
  return concat5("0x03", sidecars ? (
    // If sidecars are provided, envelope turns into a "network wrapper":
    fromHex8([serialized, blobs, commitments, proofs])
  ) : (
...
function serialize5(envelope, options = {}) {
  ...
  return concat5(serializedType4, fromHex8(serialized));
}
...
function hashType(value) {
  const { primaryType, types: types2 } = value;
  const encodedHashType = fromString4(encodeType2({ primaryType, types: types2 }));
...
export {
  ...
  entryPoint06Address,
  entryPoint07Address,
  Hex_exports2 as Hex_exports,
  PublicKey_exports2 as PublicKey_exports,
  fromHex6 as fromHex,
  Signature_exports2 as Signature_exports,
  Base64_exports2 as Base64_exports,
  WebAuthnP256_exports2 as WebAuthnP256_exports,
...
*/
//# sourceMappingURL=chunk-3NUX7GKE.js.map
